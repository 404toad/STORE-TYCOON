...
class Save:
    def __init__(self, filepath="save.json"):
    ...
    def set(self, name, value):
        """Save any variable by name."""
        self.data[name] = value
        self.save()

    def get(self, name, default=None):
        """Load any variable by name."""
...
def load_all():
    global money, days_played, today_money
    global currency, currency_wallet, currency_lose, current_price, past_price
    global sick, scam
    global explore_limit, total_explores
    global total_customers, customer_limit, served, failed, reputation
    global moneyUp_u1, fallback_u3, today_u4, alarm_u5, antiScam_u8
    global irs_encounter, icecream_broken, disaster, event_happened
    global demo, owned_upgrades, checked_today, claimed_d10

    money              = save.get("money", 0)
    days_played        = save.get("days_played", 0)
    today_money        = save.get("today_money", 0)
    currency           = save.get("currency", 0)
    currency_wallet    = save.get("currency_wallet", [])
    currency_lose      = save.get("currency_lose", 3)
    current_price      = save.get("current_price", 0)
    sick               = save.get("sick", False)
    scam               = save.get("scam", False)

    explore_limit      = save.get("explore_limit", 3)
    total_explores     = save.get("total_explores", 0)
    total_customers    = save.get("total_customers", 0)
    customer_limit     = save.get("customer_limit", 3)
    served             = save.get("served", 0)
    failed             = save.get("failed", 0)
    reputation         = save.get("reputation", 15)

    moneyUp_u1         = save.get("moneyUp_u1", False)
    fallback_u3        = save.get("fallback_u3", False)
    today_u4           = save.get("today_u4", False)
    alarm_u5           = save.get("alarm_u5", False)
    antiScam_u8        = save.get("antiScam_u8", False)

    irs_encounter      = save.get("irs_encounter", False)
    icecream_broken    = save.get("icecream_broken", False)
    disaster           = save.get("disaster", False)
    event_happened     = save.get("event_happened", True)

    demo               = save.get("demo", True)                        # CHANGE THIS TO TRUE AFTER TESTING
    owned_upgrades     = save.get("owned_upgrades", [])
    checked_today      = save.get("checked_today", False)
    claimed_d10        = save.get("claimed_d10", False)
    past_price         = save.get("past_price", 0)

def save_all():
    save.set("money", money)
    save.set("days_played", days_played)
    save.set("today_money", today_money)
    save.set("currency", currency)
    save.set("currency_wallet", currency_wallet)
    save.set("currency_lose", currency_lose)
    save.set("current_price", current_price)
    save.set("sick", sick)
    save.set("scam", scam)

    save.set("explore_limit", explore_limit)
    save.set("total_explores", total_explores)
    save.set("total_customers", total_customers)
    save.set("customer_limit", customer_limit)
    save.set("served", served)
    save.set("failed", failed)
    save.set("reputation", reputation)

    save.set("moneyUp_u1", moneyUp_u1)
    save.set("fallback_u3", fallback_u3)
    save.set("today_u4", today_u4)
    save.set("alarm_u5", alarm_u5)
    save.set("antiScam_u8", antiScam_u8)

    save.set("irs_encounter", irs_encounter)
    save.set("icecream_broken", icecream_broken)
    save.set("disaster", disaster)
    save.set("event_happened", event_happened)

    save.set("demo", demo)
    save.set("owned_upgrades", owned_upgrades)
    save.set("checked_today", checked_today)
    save.set("claimed_d10", claimed_d10)
    save.set("past_price", past_price)
...
List of commands:
    ...
    5. "mail"
          view your inbox
    6. "exit"
          closes your shop. This will save you game automatically!
    7. "done"
          advances you to the next day. you can't go back, so make sure you have done everything you wanted to!
    8. "options"
          opens the configuration menu (there isn't one yet. oops)
    9. "shop"
...
▄█ █▄█ █░▀░█ █░▀░█ █▀█ █▀▄ ░█░
          """)
    global days_played, current_price, past_price, currency_wallet, event_happened, sick, icecream_broken
...
░█░ █▀█ ▄█ █░█
    ...
    if drink_input == order_drink.lower() and flavor_input == order_flavor.lower() and size_input == order_size.lower() and food_input == order_food.lower() and timer > 0:
        timer_rundown = False
        
        print("", f"[ORDER COMPLETED] You earned ${order_payment}.")
        money += order_payment
        timer = 15
        global today_money
        ...
        timer_rundown = False
        print("", f"[ORDER FAILED WITH] The customer leaves you a bad review on {review}.")
        timer = 15
        failed += 1
        total_customers += 1
        reputation -= 1
...
def exit_game():
    save_all()
    print("Closing shop, please wait...", end="\r")
    time.sleep(2)
...
def save_data():
    with open("playerdata.txt", "w") as file:
        file.write(f"{money}\n")
        file.write(f"{past_price}\n")
        file.write(f"{days_played}\n")
        file.write(f"{}\n")
        file.write(f"{}\n")
        file.write(f"{}\n")
        file.write(f"{}\n")
